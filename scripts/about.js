...
// дальше какой-то там код Артема
...
document.addEventListener("scroll", function () {
  ...
  var elements = document.querySelectorAll(
    ".reason-container-1, .reason-container-2, .reason-container-3, .reason-container-4"
  );
  ...
  elements.forEach(function (el) {
    if (el.offsetTop < scrollPosition) {
      if (window.innerWidth <= 576) {
        ...
        if (
          el.classList.contains("reason-container-1") ||
          el.classList.contains("reason-container-3")
        ) {
